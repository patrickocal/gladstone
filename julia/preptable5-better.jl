...
tableorigdf = anztable5rr[1:numrow, Between("A", "Q7")];
tabledf = anztable5rr[1:numrow, Between("A", "Q7")];
for i in range(1, numrow)
  for j in range(1, numcol)
    tabledf[i, j] = (tabledf[i, j] != 0
                   && (tabledf[i, j]
                       * newrowtot[j] / anztable5rr[end, 1 + j]
                      ) 
                  );
  end;
end;
...
@NLobjective(ras,
             Max,
             - (sum(sum(shr[i, j] * (x[i, j] - table[i, j]) ^ rho
                        for i in range(1, numrow)
                       ) ^ (rhohat * 12)
                    for j in range(1, numcol)
                   )
