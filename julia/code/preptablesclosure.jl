...
outputdir = "julia"*sep*"output"*sep*"closure"*sep
#==============================================================================
...
regrowtot = makebasiclq(outpersecraw)[1];
(regcoltot, outgrowth) = makebasiclq(outpersecraw);
# more regional data
(source == "remplan"
...
regcoltotdiv = regcoltot;
regcoltot = regcoltot[:, Not("ANZdiv")];
for i in range(1, 6)
  push!(regcoltot, [austable8[numdiv + i, 1] p[i, 1] p[i, 2]])
end;
regrowtotdiv = regrowtot;
regrowtot = regrowtot[:, Not("ANZdiv")];
for i in range(1, 7)
  push!(regrowtot, ["`Q"*string(i) q[i, 1] q[i, 2]])
end;
# 
println("")
println(sum(regrowtot[:, 2]) - sum(regcoltot[:, 2]));
println(sum(regrowtot[:, 3]) - sum(regcoltot[:, 3]));

#=============================================================================#
# delete an oversized aluminium sector
#=============================================================================#
Alrow = findfirst(occursin.("2102", string.(table8[:, :IOIG])));
Elrow = findfirst(occursin.("Electricity", string.(table8[:, :industry])));
manrows = findall(occursin.("C", string.(table8[:, :ANZcode])));
Alrowinmanrows = findfirst(occursin.(string(Alrow), string.(manrows)));
mannames = names(table8[:, Between(:IOIG1101, :IOIG2502)]);
mannamesexAl = mannames[Not(Alrowinmanrows)]
Alfinexcontot = sum(table8[Alrow, Between(:Q3, :Q7)]);
Alcoltot = sum(table8[:, :IOIG2102]);

#Almedtot = sum(table8[Alrow, ANZdiv]);
#manfinexcontot = sum(sum(Matrix(table8[mansec, 4:end]), dims=1));
#manmedtot = sum(sum(Matrix(table8[mansec, Between(Q3, Q7)]), dims=1));
mancoltot = sum(sum(Matrix(table8[:, mannames]), dims=1));
mancolexAltot = sum(sum(Matrix(table8[:, mannamesexAl]), dims=1));
shktable8 = deepcopy(table8);
#table8shk[Alrow, ANZdiv] *=  manmedtot / Almedtot * 1000 / 4009;
shktable8[:, :IOIG2102] *= mancoltot / Alcoltot * 1000 / 4000;
shktable8[:, mannamesexAl] .*= mancoltot / mancolexAltot * 3000 / 4000;
shkdiff = sum(eachcol(table8[:, mannames])) - table8[:, :IOIG2102];
stop
#==============================================================================
RAS
==============================================================================#
...
newrowtot = regrowtot[1:numcol, 2];
newcoltot = regcoltot[1:numrow, 2];
...
# mining imports of bauxite and coal by the refineries and powerstation
