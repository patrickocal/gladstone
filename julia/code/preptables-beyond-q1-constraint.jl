...
using DataFrames, JuMP, Ipopt, DelimitedFiles, Random, PrettyTables;
...
tableshr = zeros(numrow, numcol);
for i in range(1, numrow)
  for j in range(1, numcol)
    (tabledf[i, j] != 0
     && (tabledf[i, j] = tabledf[i, j] * newrowtot[j] / sum(tabledf[:, j]);
         tableshr[i, j] = tableorigdf[i, j] / sum(tableorigdf[:, j]);
         shr[i, j] = (1 / count(!iszero, tabledf[:, j])
                      * ((rand(MersenneTwister(i + j)) - 50e-2) * 010e-2 + 1) 
                     ) ^ (- 1 / eps);
        )
    );
  end;
...
@NLobjective(ras,
             Max,
             - 1e-00 * (sum(sum(shr[i, j] * (x[i, j] - table[i, j]) ^ rho
                        for i in range(1, numrow)
                       ) ^ (rhohat * 3)
                    for j in range(1, numcol)
                   )
...
for i in range(1, numrow)
  for j in range(1, numcol)
    ...
    #@constraint(ras, x[j, q1col] <=  q1tableshr[j] * sum(x[:, q1col]) + tol);
    #@constraint(ras, x[j, q1col] >=  q1tableshr[j] * sum(x[:, q1col]) - tol);
    @constraint(ras, x[i, j] <=  tableshr[i, j] * sum(x[:, j]) + 10 * tol);
    @constraint(ras, x[i, j] >=  tableshr[i, j] * sum(x[:, j]) - 10 * tol);
  end
end;
#@constraint(ras, x[end, end] <= anztable8rr[end-1, end-1] * aggoutlq);
#infi = min(sum(newrowtot), sum(newcoltot))
#supr = max(sum(newrowtot), sum(newcoltot))
#@constraint(ras, infi <= sum(x) supr) 
set_optimizer_attribute(ras, "max_iter", 12000);
optimize!(ras);
rawsol = round.(value.(x));
for i in range(1, numrow)
  for j in range(1, numcol)
...
print(pretty_table(sol, nosubheader=true))
