...
tabledf = anztable8rr[1:numrow, Between("A", "Q7")];
for i in range(1, numrow)
  for j in range(1, numcol)
    tabledf[i, j] = (tabledf[i, j] != 0
                   && (tabledf[i, j]
                       * newrowtot[j] / anztable8rr[end, 1 + j]
                      ) 
                  );
  end;
end;
...
eps = 50e-2;
shr = (1 / count(!iszero, table)
       * (randn(numrow, numcol) * 0.1 .+ 1)) .^ (-1/eps);
rho = 1 + 1 / eps;
rhohat = 1 / rho;
@NLobjective(ras,
             Max,
             - (sum(shr[i, j] * (x[i, j] - table[i, j]) ^ rho
                      for i in range(1, numrow), j in range(1, numcol))
                  ) ^ (rhohat * 12)
            );

# Row-sums constraint - must be equal to the GFCF totals
for i in range(1, numcol)
    #@constraint(ras, sum(x[:, i]) <= newrowtot[i] + tol);
    #@constraint(ras, sum(x[:, i]) >= newrowtot[i] - tol);
    @constraint(ras, sum(x[:, i]) == newrowtot[i]);
    @constraint(ras, sum(x[:, i]) == newrowtot[i]);
end;
...
set_optimizer_attribute(ras, "max_iter", 2000)
optimize!(ras);
rawsol = value.(x);
println("worst case is ", maximum(abs.(rawsol - table)));
solraw = rawsol;
sol = DataFrame(value.(x), names(tabledf));
