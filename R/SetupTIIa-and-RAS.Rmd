...
rm(list=ls())
```
The following is my RAS algorithm. It works by acting on the "worst" difference as opposed to simultaneously for all industries. (It is easier to repeat outside of a chunk.) It is useful for identifying the problem sector.

```{r myras }
#The ras algorithm::
myras <- function(Z,U,V) {
  for (x in 1:100001){
  u <- rowSums(Z)
  j <- which(abs(u-U) == max(abs(u-U)))
#  j
  sio4$our71names[j]
#  u[j]-U[j]
  r <- abs(U[j]/u[j])
...
#  s
  Z[,i] <- Z[,i]*s
  #sum(Z[,i])-V[i]
#  if(abs(r-1) < .0001 & abs(s-1) < .0001){ break
#  x
  }
  return(Z)
}
...
sio4 <- data.table::data.table(readr::read_csv("sio4-19-preRAS.csv"))
#sio4 <- readxl::read_excel("sio4.xlsx")
sio4 <- data.table::data.table(sio4)
...
#                      "qld")
View(sio4)
#import the bottom right-hand corner
sio4brc <- readxl::read_excel("sio4-bottom-right-corner.xlsx")
sio4brc <- data.table::data.table(sio4brc)
View(sio4brc)
```
